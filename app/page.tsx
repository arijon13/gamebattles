...
export default function Home() {
  ...
  return (
    <main className="bg-gradient-to-br from-indigo-900 via-blue-900 to-purple-900 text-white py-10 flex flex-col items-center space-y-16 font-exo">
      {/* Tournaments Section */}
      <div className="flex justify-center w-full max-w-5xl px-6 space-x-4">
        {tournaments.map((tournament, index) => (
          <div
            key={index}
            className="w-1/3 rounded-lg overflow-hidden bg-gradient-to-br from-purple-800 to-blue-700 shadow-2xl p-4 flex flex-col items-center space-y-2 transform transition-transform hover:scale-105"
          >
            <img src={tournament.image} alt={tournament.name} className="w-full h-32 object-cover rounded-md shadow-md" />
            <h3 className="text-lg font-semibold text-cyan-300">{tournament.name}</h3>
            <p className="text-sm text-indigo-200">{tournament.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-center w-full max-w-5xl px-6 mb-12 relative">
        <input
          ...
          className="w-[calc(100%-2rem)] max-w-[960px] p-2.5 pl-10 rounded-full bg-indigo-800 text-white placeholder-indigo-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 border border-cyan-500 shadow-lg"
        />
        <span className="absolute left-12 top-1/2 transform -translate-y-1/2 text-cyan-400">
      ...
      <section className="relative flex flex-col items-start max-w-5xl w-full px-8 pt-4 pb-20">
          ...
          {filteredGames.slice(currentIndex, currentIndex + gamesToShow + 1).map((game, index) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black to-transparent p-2">
                <h3 className="text-cyan-200 text-center">{game.name}</h3>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="absolute top-[1.5rem] right-2 flex space-x-1">
          <button
            onClick={handlePrev}
            className="flex items-center justify-center w-8 h-8 rounded-full border border-cyan-400 bg-transparent hover:bg-purple-600 transition-transform transform active:scale-95 shadow-lg"
          >
            <svg fill="currentColor" viewBox="0 0 64 64" className="w-4 h-4 text-cyan-300 hover:text-cyan-100">
